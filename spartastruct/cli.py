...
from spartastruct.diagrams import class_diagram, dfd, er_diagram, flowchart, function_graph, module_graph
...
def init() -> None:
    ...
    from spartastruct.config import CONFIG_FILE
...
def analyze(path: str, no_llm: bool, model: str | None, output: str | None) -> None:
    ...
    with Progress(SpinnerColumn(), TextColumn("{task.description}"), console=console) as progress:
        ...
        static_diagrams: dict[str, str] = {
            key: gen(result) for key, gen in _GENERATORS.items()
        }
    ...
    console.print(Panel("\n".join(summary_lines), title="[bold]SpartaStruct Analysis Complete[/bold]"))
...
@click.option("--api-key", "api_keys", multiple=True, type=(str, str), metavar="PROVIDER KEY",
